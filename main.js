...
const style = `
        <style>
            ...
            .buttonDelete { display: inline-block; text-decoration: none;
                padding: 2px 6px; margin: 2px;
                background: #e62e1b; color: #FFF;
                border-radius: 4px; border: solid 1px #20538D; font-weight: bold; }  
            .buttonNew { display: inline-block; text-decoration: none;
                padding: 2px 6px; margin: 2px;
                background: #00c200; color: #FFF;
                border-radius: 4px; border: solid 1px #20538D; font-weight: bold; }                                    
...
const indexView = quizzes =>
    ...
    `</table>
        <a href="/quizzes/new" class="buttonNew">New Quiz</a>
    </body>
    </html>`;
...
const editView = (quiz) => {
    ...
    <body>
      <h1>Edit Quiz Number... ${quiz.id} </h1>
...
const updateController = async (req, res, next) => {
    ...
    const {question, answer} = req.body;
    if (!question) return next(new Error("Question is empty!"));
    if (!answer) return next(new Error("Answer is empty!"));
...
const destroyController = async (req, res, next) => {
    // .... introducir código
    const id = Number(req.params.id);
    if (Number.isNaN(id)) return next(new Error(`Quizz ${req.params.id} not found in DB...`));

    try {
        let n = await Quiz.destroy({where: {id}});
        if (n===0){
            return next(new Error(`Quizz ${id} not found in DB...`));
        } else {
            console.log(`Quizz ${id} remove successfully!`)
        }
        res.redirect(`/quizzes`);
    } catch (err) {
        next(err)
